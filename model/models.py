...
class New_Audio_Guided_Attention(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        self_att_feat = torch.bmm(self_spatial_att_maps, visual_feature).squeeze().reshape(batch, t_size, v_dim)  #[64,10,512]
        ...
        c_s_att_visual_feat = torch.bmm(spatial_att_maps, c_att_visual_feat).squeeze().reshape(batch, t_size, v_dim)   #[64,10,512]


        #c_s_att_a_feat = torch.bmm(spatial_att_maps, audio_feature).squeeze().reshape(batch, t_size, v_dim)   #[64,10,512]

        #

        c_s_att_visual_feat = c_s_att_visual_feat + self.beta*self_att_feat.sigmoid()*c_s_att_visual_feat
        #c_s_att_visual_feat = audio_video_query_s3c_s_att_visual_feat + self.beta * self_att_feat.sigmoid() * c_s_att_visual_feat
        # c_s_att_visual_feat = self_att_feat * c_s_att_visual_feat + c_s_att_visual_feat + self.beta * self_att_feat.sigmoid() * c_s_att_visual_feat
